...
def dna_json_from_benchling_connector(connector: BenchlingRegistryConnector, limit: int = None):
    return [benchling_to_json(d) for d in connector.all(limit)]


def available_sample_json_from_aquarium_session(session: AqSession, sample_type_names: List[str],
                                                object_type_names: List[str], page_size=1000,
                                                sample_limit: int = -1, item_limit: int = -1, sample_ids: List[int] = None) -> Dict[str, Any]:
    ...
    samples_with_inventory = merge_left(
        samples, available_items, 'id', 'sample_id', 'available_items', inplace=True, merged_only=True
    )
...
def merge_dna_json_and_sample_json(dna_json: List[Dict[str, Any]], sample_inventory: List[Dict[str, Any]]) -> \
        List[Dict[str, Any]]:
    ...
    merged = merge_left(sample_inventory, dna_json_copy, 'id', 'aquarium_sample_id', 'sequence', inplace=False, merged_only=True)
    return merged


def generate_inventory_json(connector: BenchlingRegistryConnector,
                            session: AqSession,
                            aquarium_dna_types: dict,
                            page_size=1000,
                            dna_limit: Union[int, None] = None,
                            sample_limit: int = -1, item_limit: int = -1):
    """
    ...
    for sample_type_name, types in aquarium_dna_types.items():
        ...
        _sample_json = available_sample_json_from_aquarium_session(session, sample_type_names=[sample_type_name],
