...
from aqbt.utils import remove_none_values
...
class DesignFactory(object):
    ...
    def _record_to_row(self, record: SeqRecord) -> pd.Series:
        """Using a Record, find the row in the inventory df
        that corresponds to the LIMS_ID in the record.annotations"""
    ...
    def _record_to_sample(self, record: SeqRecord) -> Sample:
        """
        Using a Sample, find the row in the inventory df that corresponds
        to the LIMS_ID in the record.annotations.
    ...
    def _record_seq_to_df(self, record: SeqRecord) -> pd.DataFrame:
        """
        Using the sha1 hash of the record sequence, find the rows
        in the inventory df that matches the provided record.
    ...
    def _new_sample_name(qk, mtype):
        """
        Create a new sample name using the query_key and type_name. Adds
        a unique string value to the end of each name.
